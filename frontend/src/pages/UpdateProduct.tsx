...
function UpdateProduct() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (productId) {
        await ProductService.updateProduct(parseInt(productId), { ...product, imageUrl });
  ...
  return (
    <Box sx={{ maxWidth: 500, margin: 'auto', mt: 5, p: 3, boxShadow: 3, borderRadius: 2, backgroundColor: 'white' }}>
      ...
      <form onSubmit={handleSubmit}>
          ...
          key === 'imageUrl' ? null : typeof value === 'boolean' ? (
            <FormControlLabel
              ...
              control={
                <Checkbox
                  name={key}
                  checked={product[key as keyof typeof product] as boolean} // Bind checked to state
        ...
        <Button type="submit" variant="contained" color="primary" fullWidth>Update Product</Button>
      </form>
    </Box>
  );
}
